...
Recommended repository name: `safetynet`
...
SafetyNet lets a user choose a meeting location, set an expected arrival time, customize their safe zone, and add up to 5 trusted contact emails. When the time arrives, the app checks the user's current GPS position:

- If the user is inside the selected safe zone (20m to 2km), alerts stop.
- If the user is outside the safe zone, the trusted contacts receive an email with the user's current location.
...
- Modern, attractive, and highly usable interface
- Google Maps location picker with place search
- Customizable safe-zone circle (adjustable from 20m to 2km) around the selected meeting point
- Browser geolocation tracking
- Scheduled check-in based on the selected time
- Emergency email alerts to up to 5 contacts with live Google Maps coordinates
...
2. The app draws a safe zone around that location, customized by the user (20m to 2km).
3. The user sets a meeting time and enters up to 5 trusted contact emails.
...
6. If the user is outside the safe zone, the app sends an email alert to the trusted contacts with their current coordinates and Google Maps link.
